...
def generate_daily_report_from_df(csv_path: str, df: pd.DataFrame):
    ...
    # 3. 仿微博/图片格式的“视频段落战报” Sheet (精准匹配每天 19:50)
    paragraph_rows = []
    for bvid, g in detail_df.groupby("BV号", sort=False):
        meta = g.iloc[-1]
    ...
    with pd.ExcelWriter(report_xlsx, engine="openpyxl") as writer:
        ...
        paragraph_df.drop(columns=["原始顺序"]).to_excel(writer, index=False, sheet_name="3. 每日战报文案汇总")
        
        worksheet = writer.sheets["3. 每日战报文案汇总"]
...
def run_collection_task(session, input_path, csv_path):
    ...
    items = read_inputs_from_file(input_path)
    if not items:
...
def main():
    ...
    print("=== B站数据监控与定时智能采集脚本已启动 ==-> 策略：整点、半点、以及每天 19:50 自动触发 ===")
