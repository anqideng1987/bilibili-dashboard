...
st.markdown("""
    <div class="bili-header">
        ...
        <div class="bili-subtitle">✨ 双列紧凑网格布局 | 冲刺 100w 目标</div>
    </div>
""", unsafe_allow_html=True)
...
def render_video_card(idx, row, target_views):
    ...
    with st.container():
        ...
        # 带小兔子的进度条
        st.markdown(f"<div style='font-size: 12px; color: #D46A92; margin-bottom: -15px;'>🐰 冲刺进度：{percent_str} (目标 100w)</div>", unsafe_allow_html=True)
        st.progress(percent)
        st.write("")
