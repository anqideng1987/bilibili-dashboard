...
st.markdown("""
    ...
    .stat-box { background: #FFFFFF; border-radius: 10px; padding: 8px 12px; text-align: center; border: 1px solid #FFE4ED; }
    .stat-label { font-size: 12px; color: #757575; margin-bottom: 2px; }
    .stat-value { font-size: 18px; font-weight: 800; color: #D46A92; }
...
if not valid_excel_files:
    ...
    try:
        ...
        # 1. 优先尝试从“半小时”或“一小时”明细表中读取具体的“采集时间”
        df_time_source = None
        if "3. 半小时间隔明细" in xls.sheet_names:
            df_time_source = pd.read_excel(latest_file, sheet_name="3. 半小时间隔明细")
        elif "2. 一小时间隔明细" in xls.sheet_names:
            df_time_source = pd.read_excel(latest_file, sheet_name="2. 一小时间隔明细")
            
        latest_timestamp = None
        if df_time_source is not None and not df_time_source.empty and "采集时间" in df_time_source.columns:
            df_time_source["采集时间"] = pd.to_datetime(df_time_source["采集时间"], errors='coerce')
            valid_times = df_time_source["采集时间"].dropna()
            if not valid_times.empty:
                latest_timestamp = valid_times.max().strftime('%Y-%m-%d %H:%M:%S')
                
        # 如果明细表里没有采集时间，退回到文件的最后修改时间
        if not latest_timestamp:
            latest_timestamp = datetime.datetime.fromtimestamp(os.path.getmtime(latest_file)).strftime('%Y-%m-%d %H:%M:%S')

        # 2. 读取第一个 Sheet（综合分析汇总）展示主体数据
        ...
        if not df.empty:
            ...
            if "BV号" not in df.columns:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    st.markdown(f'<div class="stat-box"><div class="stat-label">📌 监控视频总数</div><div class="stat-value">{total_videos} 个</div></div>', unsafe_allow_html=True)
                with c2:
                    st.markdown(f'<div class="stat-box"><div class="stat-label">🔥 累计总播放量</div><div class="stat-value">{total_views:,}</div></div>', unsafe_allow_html=True)
                ...
                # 最下方显示表内真实的最新采集时间
                st.markdown("---")
                st.markdown(f"<div style='text-align: center; color: #888888; font-size: 13px;'>✨ 数据源: {os.path.basename(latest_file)} &nbsp;|&nbsp; 🕒 数据最后采集更新时间: <b>{latest_timestamp}</b> ✨</div>", unsafe_allow_html=True)
