...
def get_bilibili_data(bv_list):
    """使用 DrissionPage 模拟真实浏览器抓取 B 站视频实时数据，防止 412 风控"""
    co = ChromiumOptions()
    co.headless(True)
        ...
        for idx, bvid in enumerate(bv_list, start=1):
            ...
            try:
                ...
                if res and res.get("is_valid"):
                    ...
                    print(f"  └─ [{idx}/{len(bv_list)}] ⚠️ 无法获取正常页面: {bvid}")
...
def update_excel_file(df_new_records, bv_order, is_hourly_point):
    """读写 Excel 文件，更新半小时与一小时间隔明细"""
    ...
    df_rules = pd.DataFrame([
        ...
        {"指标": "币赞比", "解读": "投币/点赞。>15%说明内容极硬核；反映粉丝忠忠诚度和内容深度。"},
    ...
    if os.path.exists(target_excel):
        print(f"📖 追加写入文件: {target_excel}")
        try:
            df_h1 = pd.read_excel(target_excel, sheet_name='2. 一小时间隔明细')
            df_h05 = pd.read_excel(target_excel, sheet_name='3. 半小时间隔明细')
            df_h1 = df_h1[[c for c in standard_cols if c in df_h1.columns]]
            df_h05 = df_h05[[c for c in standard_cols if c in df_h05.columns]]
        except Exception as e:
            print(f"⚠️ 读取已有 sheet 异常，重建表格: {e}")
            df_h1 = pd.DataFrame(columns=standard_cols)
            df_h05 = pd.DataFrame(columns=standard_cols)
    else:
        print(f"📄 新建今日文件: {target_excel}")
        df_h1 = pd.DataFrame(columns=standard_cols)
        df_h05 = pd.DataFrame(columns=standard_cols)
    ...
    for _, row in df_new_records.iterrows():
        ...
        # 2. 更新一小时间隔明细（仅整点时间才追加，如果首条数据可作为基准写入）
        bvid_h1_history = df_h1[df_h1["BV号"] == bvid]
        if is_hourly_point or bvid_h1_history.empty:
    ...
    # 汇总以半小时最新数据为准
    for bvid, group in df_h05.groupby("BV号"):
    ...
    print(f"🎉 报表导出完成: {target_excel}")

def get_seconds_until_next_tick():
    """精确计算距离下一个整点 (:00) 或半点 (:30) 所需等待的秒数"""
    now = datetime.now()
    if now.minute < 30:
    ...
    return sleep_seconds, next_tick

def start_precise_scheduled_loop():
    txt_file = get_file_path("bv_list.txt")
    print("🚀 Bilibili 自动追踪脚本启动！已开启整点/半点对齐模式。")
    
    while True:
        now = datetime.now()
        # 只要当前分钟在 58~02 之间就判定为整点（允许2分钟误差）
        is_hourly_point = (now.minute >= 58 or now.minute <= 2)
        
        if os.path.exists(txt_file):
            with open(txt_file, "r", encoding="utf-8") as f:
                bvs = extract_bv_ids(f.read())
        else:
            print(f"⚠️ 找不到 {txt_file}，请放置对应文件。")
            bvs = []
            
        if bvs:
            df_new = get_bilibili_data(bvs)
            update_excel_file(df_new, bv_order=bvs, is_hourly_point=is_hourly_point)
        else:
            print("⚠️ 未找到有效 BV 号，跳过本次采集。")
            
        # 计算距离下一次整点/半点的等待时间
        sleep_time, next_time = get_seconds_until_next_tick()
        print(f"\n💤 本期采集完毕，已对齐时钟。下一次定时采集将在 [{next_time.strftime('%H:%M:%S')}]（等待 {int(sleep_time)} 秒）...")
        time.sleep(sleep_time)

if __name__ == "__main__":
    start_precise_scheduled_loop()